<x-app-layout>
    <div class="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-extrabold text-gray-900 mb-6">
            <span class="text-indigo-600">Admin Panel:</span> Edit User: {{ $user->name }}
        </h1>

        {{-- SPATIE FIX: Get the user's current role name BEFORE form rendering --}}
        @php
            // $user->getRoleNames() returns a Collection of role names. We get the first one.
            ...
            <form method="POST" action="{{ route('users.update', $user->id) }}"> 
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="CNIC" id="CNIC" value="{{ old('CNIC', $user->CNIC) }}" required
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        @error('CNIC') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <h3 class="text-lg font-semibold border-t pt-6 mt-6">Change Password (Optional)</h3>
